...
$(function(){
	...
	// 적립 상세조회 이벤트 
	if("${saveCheck.checkPeriod }" == "") { // 1개월 선택시 그대로 1개월 checked
		$("#saveRadio-1").attr("checked", "checked");
	}
	
	if("${saveCheck.checkType }" == "") { // 전체 선택시 그대로 전체 checked
		$("#saveRadio-11").attr("checked", "checked");
	}

	if("${saveCheck.checkType }" == 1) { // 전체 선택시 그대로 전체 checked
		$("#saveRadio-11").attr("checked", "checked");
	}
	
	if("${saveCheck.checkType }" == 2) { // 이젠항공 선택시 그대로 이젠항공 checked
		$("#saveRadio-12").attr("checked", "checked");
	}
	
	if("${saveCheck.checkType }" == 3) { // 쇼핑몰 선택시 그대로 쇼핑몰 checked
		$("#saveRadio-13").attr("checked", "checked");
	}
	
	if("${saveCheck.checkPeriod }" == 12) { // 1년 선택시 그대로 1년 checked
		$("#saveRadio-3").attr("checked", "checked");
	}
	
	if("${saveCheck.checkPeriod }" == 6) { // 6개월 선택시 그대로 6개월 checked
		$("#saveRadio-2").attr("checked", "checked");
	}
	
	if("${saveCheck.checkPeriod }" == 1) { // 1개월 선택시 그대로 1개월 checked
		$("#saveRadio-1").attr("checked", "checked");
	}
	
	if("${saveCheck.checkPeriod }" == 99) { // 직접입력 선택시 그대로 직접입력 checked
	...
	$(".saveRadio-1").click(function(){ // 조회기간 변경시 submit
		$("#checkSavePeriodType").submit();
	});
	
	$(".saveRadio-11").click(function(){ // 조회유형 변경시 submit
		$("#checkSavePeriodType").submit();
	});
...
<div class="container">
	...
	<h1>마일리지 상세조회 (적립)</h1>
	<h3>기간과 유형을 선택하여 적립한 마일리지를 확인할 수 있습니다.</h3>
		<div class="useBtn" style="text-align: right;">
			<button id="useBtn" style="color: #b30000;">사용 마일리지 <i class="fa fa-chevron-right"></i></button>
		</div>
	
		<div class="tab">
			<button class="tablinks active" id="openTab1">적립</button>
		</div>
